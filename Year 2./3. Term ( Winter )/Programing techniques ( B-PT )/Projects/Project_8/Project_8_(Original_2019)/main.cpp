...
list<int>::const_iterator findValue(const list<int> &data, int value) noexcept {
    list<int>::const_iterator first = find(data.cbegin(), data.cend(), value);
    if (first != data.cend())
        return first;
    else
        return data.cend();
...
list<int>::const_iterator findInRange200to400(const list<int> &data) noexcept {
    list<int>::const_iterator in_range = find_if(data.cbegin(), data.cend(), isInRange200to400);
    if (in_range != data.cend())
        return in_range;
    else
        return data.cend();
}
...
list<int> add(const list<int> &data1, const list<int> &data2) noexcept {
    list<int> new_list(data1.size());
    if (data1.size() == data2.size()) {
        transform(data1.begin(), data1.end(), data2.begin(), new_list.begin(), plus<int>());
    }
    return new_list;
}
