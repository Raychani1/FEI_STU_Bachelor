...
queue<int> reverse(const list<int> &data) noexcept {
    queue<int> reversed;
    for (list<int>::const_reverse_iterator rit = data.crbegin(); rit != data.crend(); ++rit) {
        reversed.push(*rit);
    }
    return reversed;
}
...
void reverse(queue<int> &data) noexcept {
    stack<int> my_stack;
    while (!data.empty()) {
        my_stack.push(data.front());
        data.pop();
    }
    while (!my_stack.empty()) {
        data.push(my_stack.top());
        my_stack.pop();
    }
}
...
list<string> translate(const map<string, string> &translator, const list<string> &sentence) noexcept {
    map<string, string>::const_iterator miterator;
    list<string> translated;
    for (list<string>::const_iterator literator = sentence.cbegin(); literator != sentence.cend(); ++literator) {
        miterator = translator.find(*literator);
        if (miterator != translator.end()) {
            translated.push_back(miterator->second);
        } else
...
bool areParenthesisPaired(const string &mathExpression) noexcept {
    bool paired = false;
    int parenthesis_start = 0, parenthesis_end = 0, square_brackets_start = 0, square_brackets_end = 0, curly_brackets_start = 0, curly_brackets_end = 0, angle_brackets_start = 0, angle_brackets_end = 0;
    for (auto ch : mathExpression) {
        switch (ch) {
            case '(':
                ++parenthesis_start;
                break;
            ...
            case '>':
                ++angle_brackets_end;
                if (angle_brackets_end > angle_brackets_start) {
                    return paired;
                }
                break;
            case '{':
            ...
            case '}':
                ++curly_brackets_end;
                if (curly_brackets_end > curly_brackets_start) {
                    return paired;
                }
                break;
            case '[':
            ...
            case ']':
                ++square_brackets_end;
                if (square_brackets_end > square_brackets_start) {
                    return paired;
                }
                break;
        }
    }
    if (parenthesis_start == parenthesis_end && square_brackets_start == square_brackets_end &&
        curly_brackets_start == curly_brackets_end && angle_brackets_start == angle_brackets_end) {
        paired = true;
    }
    return paired;
}
