...
size_t getPivotIndex(const int *data, const size_t low, const size_t high)
{
    ...
    const size_t medium = (high + low) / 2;
    if ((data[low] < data[medium] && data[medium] < data[high-1]) || (data[high-1] < data[medium] && data[medium] < data[low])) {
        return medium;
    }
    else if ((data[medium] < data[low] && data[low] < data[high-1])|| (data[high-1] < data[low] && data[low] < data[medium])) {
        return low;
    }
    else
        return high -1 ;
...
size_t partition(int *data, const size_t pivot, const size_t low, const size_t high)
{
    ...
    for (int i = low; i < high-1; ++i) {
        if (data[i] >= data[high-1]) {
            swap(data + insert_index, data + i);
            ++insert_index;
        }
    }
    swap(data + (high-1), data + insert_index);
    return insert_index;
}
