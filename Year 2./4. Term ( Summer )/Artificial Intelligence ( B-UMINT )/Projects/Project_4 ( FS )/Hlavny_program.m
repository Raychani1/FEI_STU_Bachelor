...
hranica_pa=30;      % obmedzenie vstupu pre FS
...
% f=readfis('krizovatkaA.fis');
f=readfis('krizovatkaB.fis');
...
for cyk = 1:20 % hlavny cyklus
  ...
  more_cars_on_red=max(A2,A3);
  Interval=round(evalfis(f,[A1,more_cars_on_red]));
  ...
  more_cars_on_red=max(A1,A3);
  Interval=round(evalfis(f,[A2,more_cars_on_red]));
   ...
   % spocitanie aut z pruhov do cyklov
    ...
    %% vypocet intervalu zelenej z fuzzy systemu
  more_cars_on_red=max(A1,A2);
  Interval=round(evalfis(f,[A3,more_cars_on_red]));
